...
class StockItemMovementPermissionMixin(WarehousePermissionMixin):
    """Mixin to check permissions for stock item movements"""
    def _check_permissions(self):
        ...
        if stock_movement.status == StockMovement.Status.CONFIRMED:
            raise PermissionDenied("Cannot manage items in confirmed stock movements.")
...
class StockItemMovementCreateView(LoginRequiredMixin, StockItemMovementPermissionMixin, CreateView):
    model = StockMovementItem
    ...
    def form_invalid(self, form):
        ...
        response['HX-Retarget'] = '#movement-item-form'
        response['HX-Reswap'] = 'innerHTML'
        print("Form invalid:", form.errors)
        return response

class StockItemMovementUpdateView(LoginRequiredMixin, StockItemMovementPermissionMixin, UpdateView):
    model = StockMovementItem
    form_class = StockMovementItemForm
    template_name = 'inventory/item-movement/partials/item-movement-form.html'
...
class StockItemMovementDeleteView(LoginRequiredMixin, StockItemMovementPermissionMixin, DeleteView):
    permission_required_base = 'delete_stockmovementitem'
    # Only allow DELETE method
    http_method_names = ['delete']
    
    def delete(self, request, stock_movement_id, pk):
        try:
            movement_item = get_object_or_404(StockMovementItem, pk=pk)
            
            self._check_permissions()            
            movement_item.delete()
            response = HttpResponse(status=200)  # No content
            response['HX-Trigger'] = 'success'
            return response
        except StockMovementItem.DoesNotExist:
            return HttpResponse(status=404, content="Movement item not found.")
